...
class World(object):
    ...
    def handle_keyup(self,event):
        if (event.key == K_LEFT):
            ...
            p =(Vector(* world.player.rect.center)- world.player.facing*2).to_position()
            bullet = Bullet(p,v,world.player.motion.magnitude()+10)
...
def main():
    ...
    # main loop
    update1 = Thread(target=update_s)
    update1.start()
    update2 = Thread(target=update_collision)
    update2.start()
    while world.running:
